...
为了提升ARM架构下的性能，我们进行了以下优化：

1. **内存对齐优化**：
   - 将Pair结构体修改为带有16字节内存对齐的结构体，提高ARM64架构下的访问效率

2. **字符串操作优化**：
   - 优化BPE模型构建过程中的字符串格式化操作，使用预分配内存减少分配次数
   - 优化字节回退处理中的字符串生成，使用静态缓冲区替代format!调用

3. **缓存大小调整**：
   - 为ARM64架构调整默认缓存大小，从10,000增加到16,384，利用ARM更大的L2缓存

4. **NEON指令支持**：
   - 添加了ARM NEON指令优化的字符串处理函数，提高特殊字符检测效率

5. **并行处理优化**：
   - 为ARM架构设计了更保守的线程数配置，避免过多的上下文切换开销
   - 提供了optimal_num_threads函数，根据架构自动选择最佳线程数

### 8.2 优化效果

优化后，huggingface tokenizers在ARM架构上的性能有了一定提升：

- **中等数据量（8线程）**：提升约3.4%（22.57 → 23.35 MB/s）
- **大量小文档（8线程）**：提升约3.4%（13.62 → 14.08 MB/s）
- **大量小文档（1线程）**：提升约4.3%（6.27 → 6.54 MB/s）

这些优化主要针对ARM架构的特性进行了调整，包括内存访问模式、缓存利用率和线程调度策略，以充分发挥ARM处理器的性能潜力。
